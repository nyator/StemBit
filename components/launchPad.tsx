...
export default function LaunchPadComponent({ selectKey, isPlaying, onPress }: LaunchPadComponentProp) {
    return (
        <TouchableOpacity
            className={` w-4/12 h-32 rounded-2xl flex justify-center items-center border-dashed  ${isPlaying ? "border-2 border-green-300 shadow-sm bg-accent shadow-accent" : "bg-white border-2 border-gray-300 shadow-sm shadow-gray-400"}`}
            onPress={onPress}
        >
